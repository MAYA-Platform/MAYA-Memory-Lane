 ...
 *
 * Jev (TypeSafe jev-1.13 via Merge Gateway, POST /v1/decisions) is a
 * System One decision model: it takes a state + typed questions and returns
 * calibrated scores — no chat, no tokens-for-prose. JevRank uses it as a
 * relevance classifier over the candidates full-text search already found:
 *
 ...
 * Timeout 3s. Key from MERGE_API_KEY env, else the Hermes config.yaml merge
 * provider entry (same sourcing as hermes/scripts/jev_client.py). The key is
 * never logged.
 */

const JEV_URL = 'https://api-gateway.merge.dev/v1/decisions';
const JEV_MODEL = 'typesafe/jev-1.13'; // decisions-only: absent from /v1/models by design
...
/**
 * Resolve the Merge key. Env first, then the Hermes config.yaml merge
 * provider entry (the line reads `api_key: mg_...`). Never logs the key.
 */
