 ...
 *
 * Provider routing (recommended model = deepseek v4 flash via DeepSeek):
 *
 *   1. env overrides:   MEMORY_LANE_API_KEY, MEMORY_LANE_BASE_URL,
 *                       MEMORY_LANE_MODEL
 *   2. Hermes config:   HERMES_HOME/config.yaml -> providers.deepseek
 ...
 * Returns { apiKey, baseUrl, model, backend } where backend is 'deepseek' or 'ollama'.
 */
export function resolveExtractConfig() {
  ...
  // 2. Hermes config.yaml -> providers.deepseek (yaml sub-parse; flat keys only).
  try {
    ...
    if (fs.existsSync(cfgPath)) {
      ...
      const dsBlock = extractYamlSection(yaml, 'deepseek');
      const key = dsBlock.api_key || null;
      if (key) {
        return {
          apiKey: key,
          baseUrl: dsBlock.base_url || baseUrl || 'https://api.deepseek.com/v1',
 ...
 * to a named 2-space-indented section (e.g. `deepseek:` inside `providers:`).
...
export async function extractFacts(transcript, { maxLen = 24000, timeoutMs = 120000 } = {}) {
  ...
  try {
    if (cfg.backend === 'deepseek') {
      return await extractViaDeepSeek(cfg, text, timeoutMs);
    }
...
async function extractViaDeepSeek(cfg, text, timeoutMs) {
  // Up to 3 attempts: the DeepSeek endpoint occasionally answers a prompt with a
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (!res.ok) {
      return { ok: false, facts: [], error: `deepseek HTTP ${res.status}` };
    }
    const data = await res.json();
    const msg = data.choices && data.choices[0] && data.choices[0].message;
    let content = (msg && msg.content) || '';
    // DeepSeek is a thinking model: it occasionally routes the whole
    ...
    if (attempt === 2) {
      return { ok: false, facts: [], error: 'deepseek returned empty content after 3 attempts' };
    }
    await new Promise((r) => setTimeout(r, 500 * (attempt + 1)));
  }
  return { ok: false, facts: [], error: 'deepseek returned empty content' };
}
