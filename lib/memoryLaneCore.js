...
function getFtsIndex(library) {
  ...
  const insert = db.prepare(
    'INSERT INTO blocks_fts (body, block_id, lib_id, display) VALUES (?, ?, ?, ?)'
  );
  ...
  for (const entry of library.blocks) {
    ...
    insert.run(block.raw, entry.block_id, entry.lib_id, display);
  }

  const index = { db, select: db.prepare(
    'SELECT block_id, lib_id, bm25(blocks_fts, 5.0, 1.0, 3.0) AS rank ' +
