...
export function search(library, query, { limit = 25 } = {}) {
  ...
  // --- Graph lane (HippoRAG-patterns-lite) + cross-boost fusion ---
  // The graph walk scores blocks by entity co-occurrence (bounded 2-hop,
  // IDF-weighted). Fusion is conservative, two moves:
  //   1. CROSS-BOOST: within the lexical top-10, blocks the graph also
  //      ranks in its top-10 move to the front (stable order kept). The
  //      candidate set never changes — the graph only re-sorts evidence
  //      lexical search already found. Measured on LongMemEval oracle:
  //      recall@5 61.08% -> 62.93%, recall@10 unchanged.
  //   2. APPEND: graph-only hits (blocks the graph surfaces but the words
  //      miss) go AFTER all lexical matches, filling thin result sets.
  try {
    const graph = graphSearch(library, q, { limit: limit * 2 });
    if (graph.count > 0 && matches.length > 0) {
      const gTop = new Set(graph.matches.slice(0, 10).map((m) => m.lib_id));
      const head = matches.slice(0, 10);
      const boosted = head.filter((m) => gTop.has(m.lib_id));
      const plain = head.filter((m) => !gTop.has(m.lib_id));
      const tail = matches.slice(10);
      // Append graph-only hits after all lexical matches (thin-set fill).
      const present = new Set(matches.map((m) => m.lib_id));
      const entryByLib = new Map(library.blocks.map((b) => [b.lib_id, b]));
      const appended = [];
      for (const g of graph.matches) {
        if (present.has(g.lib_id)) continue;
        if (matches.length + appended.length >= limit) break;
        const entry = entryByLib.get(g.lib_id);
        if (!entry) continue;
        const block = readBlock(library, g.lib_id);
        if (!block || !block.present) continue;
        appended.push({
          lib_id: entry.lib_id,
          block_id: entry.block_id,
      ...
      matches.length = 0;
      matches.push(...boosted, ...plain, ...tail, ...appended);
    }
  } catch {
    // Graph lane must never break lexical search — swallow and continue.
  }
...
export function loadLibrary(rootDir) {
  ...
  if (!fs.existsSync(manifestPath)) {
    return { ok: false, reason: `MANIFEST.json not found at ${manifestPath} — this folder is not a Memory Lane library yet. To bring an existing folder of notes (an Obsidian vault, a docs tree) into Memory Lane, run: node tools/import-vault.mjs "<your folder>" --library "${manifestPath.replace(/[\\/]MANIFEST\.json$/, '')}" — it creates the library and imports every markdown note as a searchable, chain-linked block. A Memory Lane library is its own folder with its own MANIFEST.json; an external vault is imported, not opened in place.` };
  }
...
export function verifyChain(library) {
  ...
  // Three-state verdict: green (intact),
...
/**
 * --- Write path (v3, 2026-08-05) ---
 *
 * The read-only library grows up: createBlockText() + appendBlock() seal a
 * new block onto the chain with the exact registrar semantics used by the
 * continuity library:
 *
 *   - lib_id = max(existing lib_id) + 1   (never total_blocks + 1)
 *   - prev_block_id / prev_sha256 link to the current tail block
 *   - 6 blocks per shelf, shelf-NNN layout, block-NNN.md filenames
 *   - master MANIFEST.json + per-shelf shelf-manifest.json both updated
 *   - same-content dedup (identical sha256 is skipped, not duplicated)
 *
 * The chain stays tamper-evident: every new block records the hash of its
 * predecessor, so verifyChain() walks the whole linked list exactly as it
 * does for hand-sealed blocks. Files remain the source of truth.
 */

/**
 * Build the markdown text of a block file (frontmatter + body + facts).
 * Mirrors the sample-library format so hand-sealed and auto-sealed blocks
...
export function createBlockText({
  ...
  return [
    ...
    `block_id: ${clean(blockId)}`,
    `display_name: "${clean(displayName)}"`,
    `block_number: ${blockNumber}`,
    `status: ${status}`,
    `timestamp: "${ts}"`,
    `previous_block: ${clean(previousBlock)}`,
    '---',
    '',
    `# ${clean(title) || 'Memory block'}`,
    '',
    String(body).trim(),
    sourceSection.trim(),
    factsSection.trim(),
...
/**
 * Append a new block onto the library chain. Reads the current manifest,
 * allocates max(lib_id)+1, links to the tail block, writes the file to its
 * shelf, then updates the shelf manifest and the master manifest.
 *
 * Returns { ok, lib_id, block_id, sha256, prev_block_id, prev_sha256,
 * skipped } — skipped=true when identical content is already registered.
 */
export function appendBlock(rootDir, opts = {}) {
  const {
    title = 'Memory block',
    body = '',
    facts = [],
  ...
  // Allocate next lib_id — max+1, never count+1 (registrar bugfix 2026-08-05).
  const existingIds = manifest.blocks.map((b) => Number(b.lib_id) || 0);
  const libId = existingIds.length ? Math.max(...existingIds) + 1 : 1;

  // Link to the current tail block (last entry in manifest order).
  const tail = manifest.blocks.length ? manifest.blocks[manifest.blocks.length - 1] : null;
  const prevBlockId = tail ? tail.block_id : null;
  const prevSha256 = tail ? tail.sha256 : null;

  const canonical = `cb_${String(lineage).replace(/[^a-z0-9_-]/gi, '_').toLowerCase()}_${String(libId).padStart(3, '0')}`;
  const blockId = canonical;
  const ts = timestamp || new Date().toISOString();
  // Dedup key: stable content fingerprint EXCLUDING both the live timestamp
  // AND the extracted facts (LLM extraction is non-deterministic, so facts
  // must never gate dedup). Same raw input -> same key -> skipped.
  const dedupKey = sha256(JSON.stringify({ title, body, source, lineage }));
  const text = createBlockText({
    ...
    status,
    timestamp: ts,
    previousBlock: prevBlockId || 'none',
    title,
    body,
    facts,
    source
  ...
  // Same-content dedup: identical dedup_key (or legacy sha256 for blocks
  // sealed before dedup_key existed) already registered -> skip.
  for (const b of manifest.blocks) {
    if ((b.dedup_key && b.dedup_key === dedupKey) || b.sha256 === sha) {
      return { ok: true, skipped: true, lib_id: b.lib_id, block_id: b.block_id, sha256: sha };
    }
  }

  // Shelf layout: 6 blocks per shelf.
  const shelfNum = Math.floor((libId - 1) / 6) + 1;
  const shelfPos = ((libId - 1) % 6) + 1;
  const shelf = `shelf-${String(shelfNum).padStart(3, '0')}`;
  const filename = `block-${String(libId).padStart(3, '0')}.md`;
  const shelfDir = path.join(rootDir, 'shelves', shelf);
  fs.mkdirSync(shelfDir, { recursive: true });
  fs.writeFileSync(path.join(shelfDir, filename), text, 'utf8');
  ...
  // --- Update shelf manifest ---
  const smfPath = path.join(shelfDir, 'shelf-manifest.json');
  const smf = readJsonSafe(smfPath) || { shelf_id: shelf, block_count: 0, block_range: '', blocks: [] };
  smf.blocks.push(entry);
  smf.block_count = smf.blocks.length;
  const first = smf.blocks[0].lib_id;
  const last = smf.blocks[smf.blocks.length - 1].lib_id;
  smf.block_range = `blocks ${String(first).padStart(3, '0')}-${String(last).padStart(3, '0')}`;
  fs.writeFileSync(smfPath, JSON.stringify(smf, null, 2) + '\n', 'utf8');

  // --- Update master manifest ---
  manifest.blocks.push(entry);
  manifest.total_blocks = libId;
  manifest.total_shelves = Math.max(Number(manifest.total_shelves) || 0, shelfNum);
  manifest.updated = ts;
  if (!Array.isArray(manifest.shelves)) manifest.shelves = [];
  if (!manifest.shelves.includes(shelf)) manifest.shelves.push(shelf);
  if (manifest.lineages && !Array.isArray(manifest.lineages)) manifest.lineages = [];
  if (manifest.lineages && !manifest.lineages.includes(lineage)) manifest.lineages.push(lineage);
  if (!Array.isArray(manifest.volumes)) manifest.volumes = [];
  if (!manifest.volumes.includes('volume-memory-layer')) manifest.volumes.push('volume-memory-layer');
